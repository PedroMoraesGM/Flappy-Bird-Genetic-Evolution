...
current_pool = []
fitness = []
total_models = 20  # Increased from 10 to 20 for better genetic diversity
...
def model_mutate(weights):
    mutation_rate = 0.3  # Increased from 0.15 to 0.3
    mutation_strength = 0.8  # Increased from 0.5 to 0.8
...
def showGameOverScreen(crashInfo):
    ...
    # Keep top 20% as elites (unchanged)
    elite_count = max(1, total_models // 5)
    ...
    while len(new_weights) < total_models:
        # Tournament selection for better diversity
        parent1_idx = tournament_selection(fitness, 3)
        parent2_idx = tournament_selection(fitness, 3)
        
        # Ensure parents are different
        while parent2_idx == parent1_idx:
            parent2_idx = tournament_selection(fitness, 3)
        
        offspring = model_crossover(parent1_idx, parent2_idx)
        
        # Add both offspring with mutation
        if len(new_weights) < total_models:
            new_weights.append(model_mutate(offspring[0]))
        if len(new_weights) < total_models:
            new_weights.append(model_mutate(offspring[1]))
...
def tournament_selection(fitness_list, tournament_size):
    """Select parent using tournament selection for better diversity"""
    tournament_indices = random.sample(range(len(fitness_list)), tournament_size)
    best_idx = tournament_indices[0]
    best_fitness = fitness_list[best_idx]
    
    for idx in tournament_indices[1:]:
        if fitness_list[idx] > best_fitness:
            best_fitness = fitness_list[idx]
            best_idx = idx
